...
public class Fragment_Delete_Purchase extends Fragment implements View.OnClickListener {
    ...
    public View onCreateView(LayoutInflater inflater,
        ...
        view = inflater.inflate(
                R.layout.fragment_delete_purchase, container, false);
        deleteAll = (Button) view.findViewById(R.id.deleteAllPurchases);
        deleteAll.setOnClickListener(this);
        searchAndDelete = (Button) view.findViewById(R.id.searchAndDelete);
        searchAndDelete.setOnClickListener(this);

        purchaseNumber = (TextView) view.findViewById(R.id.purchaseNumberDelete);
    ...
    public void onClick(View v) {
        ...
        if (v.getId() == R.id.searchAndDelete) {
            ...
            if (purchaseNumber.getText().toString().trim().length() <= 0) {
                purchaseNumber.setError(REQUIRED_MSG);
                valid = false;
            }
            if (valid) {
                values = dataSource.getPurchases(purchaseNumber.getText().toString().trim(),null,null,null);
                if (values.size() == 1) {
                    dataSource.deletePurchase(values.get(0));
                    loadHomeFragment();
                }
            }
        }else if(v.getId() == R.id.deleteAllPurchases){
            dataSource.deleteAllPurchases();
            loadHomeFragment();
        }
    }
